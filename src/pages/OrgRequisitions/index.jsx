...
import {
  Box, Typography, CircularProgress, Alert, Grid, Chip,
  ...
  if (isError) {
    return (
      <Alert severity="error" sx={{ m: 2, borderRadius: "10px" }}>
        Failed to load requisitions: {error?.data?.message ?? error?.status ?? "Something went wrong"}
      </Alert>
    );
  }
    ...
    <Box sx={{ p: 1 }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: 2.5, flexWrap: "wrap", gap: 1.5 }}>
        ...
        <Chip
          ...
          sx={{ backgroundColor: "#fff", fontWeight: 500, border: `1px solid ${C.border}` }}
        />
      </Box>

      {jobs.length === 0 ? (
        <Box sx={{ textAlign: "center", py: 10, backgroundColor: "#fff", border: `1px solid ${C.border}`, borderRadius: "12px" }}>
          <WorkOutlineRoundedIcon sx={{ fontSize: 42, color: C.textTertiary, mb: 1.5 }} />
          <Typography sx={{ fontSize: 15, fontWeight: 600, color: C.textPrimary }}>No requisitions found</Typography>
          <Typography sx={{ fontSize: 13, color: C.textSecondary, mt: 0.5 }}>
  ...
  return (
    ...
    >
      <CardContent sx={{ p: "16px", flexGrow: 1, display: "flex", flexDirection: "column" }}>
        ...
        <Typography sx={{ fontSize: 13, fontWeight: 700, color: C.textPrimary, mb: "2px", lineHeight: 1.3 }}>{title}</Typography>
        ...
        {(func || subFunc) && (
          <Box sx={{ display: "flex", gap: 0.6, mb: "10px", flexWrap: "wrap" }}>
            {[func, subFunc].filter(Boolean).map((label) => (
              <Chip key={label} label={label} size="small" sx={{ fontSize: 10, height: 20, textTransform: "capitalize", backgroundColor: "#F3F4F6", color: C.textSecondary, "& .MuiChip-label": { px: "8px" } }} />
            ))}
          </Box>
        )}
        ...
        {createdBy && (
          <Box sx={{ display: "flex", justifyContent: "space-between", mb: "6px" }}>
            ...
            <Typography sx={{ fontSize: 11, fontWeight: 600, color: C.textPrimary }}>{createdBy}</Typography>
          </Box>
        )}
        {approver && (
          <Box sx={{ display: "flex", justifyContent: "space-between", mb: "10px" }}>
            <Typography sx={{ fontSize: 11, color: C.textSecondary }}>Approver</Typography>
            <Typography sx={{ fontSize: 11, fontWeight: 600, color: C.textPrimary }}>{approver}</Typography>
          </Box>
        )}
        ...
        {skills.length > 0 && (
          <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5, mb: "10px" }}>
            {skills.slice(0, 4).map((skill) => (
              <Chip key={skill} label={skill} size="small" variant="outlined" sx={{ fontSize: 10, height: 20, textTransform: "capitalize", borderColor: C.border, color: C.textSecondary, "& .MuiChip-label": { px: "7px" } }} />
            ))}
            {skills.length > 4 && (
              <Chip label={`+${skills.length - 4} more`} size="small" variant="outlined" sx={{ fontSize: 10, height: 20, borderColor: C.border, color: C.textSecondary, "& .MuiChip-label": { px: "7px" } }} />
            )}
          </Box>
        )}
