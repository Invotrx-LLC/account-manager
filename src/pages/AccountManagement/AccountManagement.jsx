...
import {
  ...
  ListItemButton, Checkbox,
  IconButton, Dialog,
  DialogTitle, DialogContent, DialogActions,
  Tab, Tabs, Grid, MenuItem, Snackbar, Select, FormControl, Chip,
} from "@mui/material";
...
const editFieldSx = {
  "& .MuiOutlinedInput-root": {
    ...
    "& fieldset": { borderColor: "#E8E8EC" },
    "&:hover fieldset": { borderColor: "#D1D5DB" },
    "&.Mui-focused fieldset": { borderColor: "#FF5F1F" },
  },
};
...
function ReadField({ label, value }) {
  return (
    <Box>
      <Typography fontSize={10} fontWeight={700} color="#9CA3AF" letterSpacing="0.07em"
    ...
    <Box>
      ...
      {select ? (
        <TextField select size="small" fullWidth
          ...
          sx={editFieldSx}
        >
          <MenuItem value=""><em style={{ color: "#9CA3AF", fontStyle: "normal" }}>—</em></MenuItem>
  ...
  return (
    <Box sx={{ p: 1 }}>
      ...
      <Tabs value={tab} onChange={(_, v) => setTab(v)}
        sx={{
          mb: "20px", minHeight: 36,
          "& .MuiTabs-indicator": { backgroundColor: C.accent, height: 2, borderRadius: 1 },
          "& .MuiTab-root": {
            textTransform: "none", fontWeight: 600, fontSize: 13,
            minHeight: 36, px: "14px", color: C.textSecondary,
            "&.Mui-selected": { color: C.accent },
          },
        }}
      >
        <Tab icon={<Person2OutlinedIcon sx={{ fontSize: 16 }} />} iconPosition="start" label="My Profile" />
        <Tab icon={<GroupOutlinedIcon   sx={{ fontSize: 16 }} />} iconPosition="start" label="Users" />
      </Tabs>
...
function MyProfileTab() {
  ...
  if (isLoading) return <Box display="flex" justifyContent="center" py={8}><CircularProgress sx={{ color: C.accent }} /></Box>;
  if (isError)   return <Alert severity="error" sx={{ borderRadius: "10px" }}>Failed to load profile.</Alert>;

  return (
    <Box sx={{ backgroundColor: "#fff", border: `1px solid ${C.border}`, borderRadius: "14px", overflow: "hidden" }}>
      {/* Header */}
      <Box sx={{ p: "24px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          <Avatar sx={{ width: 52, height: 52, bgcolor: C.accent, fontSize: 18, fontWeight: 700 }}>
            {getInitials(profile.full_name)}
          </Avatar>
          <Box>
            <Typography fontSize={16} fontWeight={700} color={C.textPrimary}>{profile.full_name || "—"}</Typography>
            <Typography fontSize={12} color="#9CA3AF">{profile.email}</Typography>
          </Box>
        </Box>
        {!editing ? (
          <Button size="small" startIcon={<EditOutlinedIcon sx={{ fontSize: 14 }} />} onClick={startEdit}
            sx={{ textTransform: "none", fontSize: 12, fontWeight: 600, color: C.accent, border: `1px solid ${C.accent}`, borderRadius: "8px", "&:hover": { backgroundColor: C.accentSoft } }}>
            Edit Profile
          </Button>
        ) : (
          <Box sx={{ display: "flex", gap: 1 }}>
            <Button size="small" onClick={() => { setEditing(false); setPhoneError(""); }}
              sx={{ textTransform: "none", fontSize: 12, fontWeight: 600, color: C.textSecondary, border: `1px solid ${C.border}`, borderRadius: "8px", "&:hover": { backgroundColor: "#F9FAFB" } }}>
              Cancel
            </Button>
            <Button size="small" onClick={handleSave} disabled={saving}
              startIcon={saving ? <CircularProgress size={12} sx={{ color: "#fff" }} /> : <SaveOutlinedIcon sx={{ fontSize: 14 }} />}
              sx={{ textTransform: "none", fontSize: 12, fontWeight: 600, color: "#fff", backgroundColor: C.accent, borderRadius: "8px", "&:hover": { backgroundColor: "#E54E10" }, "&:disabled": { backgroundColor: "#F3F4F6", color: "#9CA3AF" } }}>
              Save Changes
            </Button>
          </Box>
        )}
      ...
      {/* Body */}
      <Box sx={{ p: "24px" }}>
        <Typography fontSize={11} fontWeight={700} color="#9CA3AF" letterSpacing="0.08em" sx={{ textTransform: "uppercase", mb: "16px" }}>
          Basic Information
        </Typography>
          ...
          {editing ? (
            <>
              ...
              <Grid item size={{xs:12,sm:6}}>
                <PhoneEditField dialCode={dialCode} setDialCode={setDialCode} localPhone={localPhone} setLocalPhone={setLocalPhone} error={phoneError} />
              </Grid>
            ...
            <>
              <Grid item size={{xs:12,sm:6}}><ReadField label="First Name" value={profile.first_name} /></Grid>
              <Grid item size={{xs:12,sm:6}}><ReadField label="Last Name"  value={profile.last_name} /></Grid>
              <Grid item size={{xs:12,sm:6}}><ReadField label="Phone"      value={profile.phone} /></Grid>
              <Grid item size={{xs:12,sm:6}}><ReadField label="Gender"     value={profile.gender} /></Grid>
            </>
          )}
        </Grid>
        ...
        <Grid container spacing={3}>
          {editing ? (
            ...
            <>
              <Grid item size={{xs:12,sm:6}}><ReadField label="Employee ID"   value={profile.emp_id} /></Grid>
              <Grid item size={{xs:12,sm:6}}><ReadField label="Department"    value={profile.department} /></Grid>
              <Grid item size={{xs:12,sm:6}}><ReadField label="Designation"   value={profile.emp_designation} /></Grid>
              <Grid item size={{xs:12,sm:6}}><ReadField label="Employee Type" value={profile.emp_type} /></Grid>
              <Grid item size={{xs:12,sm:6}}><ReadField label="Status"        value={profile.emp_status} /></Grid>
              <Grid item size={{xs:12,sm:6}}><ReadField label="Time Zone"     value={profile.time_zone} /></Grid>
            </>
          )}
        </Grid>
      </Box>
            ...
            return (
              <Box key={user.id} sx={{
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.6 }}>
                  <AdminPanelSettingsOutlinedIcon sx={{ fontSize: 14, color: C.accent, flexShrink: 0 }} />
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.6, flexWrap: "wrap" }}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 0.4, mr: "4px" }}>
                    <BusinessIcon sx={{ fontSize: 13, color: "#9CA3AF" }} />
                    <Typography fontSize={12} color={C.textSecondary} fontWeight={600}>
                  ...
                  {orgs.slice(0, 2).map(o => (
                    <Box key={o.organisation_id} sx={{
                      ...
                      borderRadius: "6px", backgroundColor: C.accentSoft,
                  ...
                  {orgs.length > 2 && (
                    <Box sx={{
                      fontSize: 10, fontWeight: 600, px: "7px", py: "2px",
                      borderRadius: "6px", backgroundColor: "#F3F4F6", color: "#9CA3AF",
                    }}>
                      +{orgs.length - 2}
                    </Box>
                  )}
...
function AssignOrgDialog({ user, onClose }) {
  ...
  const handleClose = () => {
    setOrgSearch(""); setSelectedOrg(null); setSuccessMsg(""); onClose();
  };
    ...
    <Dialog open={!!user} onClose={handleClose} fullWidth maxWidth="sm"
      ...
      <DialogTitle sx={{ px: "24px", pt: "24px", pb: 0 }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <Box>
            <Typography fontSize={16} fontWeight={700} color={C.textPrimary}>Assign Organisation</Typography>
            <Typography fontSize={12} color="#9CA3AF" mt="2px">Select an organisation to assign to this user</Typography>
          </Box>
        ...
        {/* User pill */}
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, p: "12px", mt: "16px", backgroundColor: C.accentSoft, borderRadius: "10px" }}>
          <Avatar sx={{ width: 32, height: 32, bgcolor: C.accent, fontSize: 12, fontWeight: 700 }}>
            {getInitials(user?.full_name ?? "")}
          </Avatar>
          <Box sx={{ flex: 1, minWidth: 0 }}>
            <Typography fontSize={13} fontWeight={600} color={C.textPrimary}>{user?.full_name}</Typography>
            <Typography fontSize={11} color={C.textSecondary}>{user?.email}</Typography>
          </Box>
          <Box sx={{ fontSize: 11, fontWeight: 600, color: C.accent, whiteSpace: "nowrap" }}>
        ...
        {(user?.organisations?.length > 0) && (
          <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.6, mt: "10px" }}>
            {user.organisations.map(o => (
              <Box key={o.organisation_id} sx={{
                fontSize: 10, fontWeight: 600, px: "8px", py: "3px",
                borderRadius: "6px", backgroundColor: "#F3F4F6", color: C.textSecondary,
              }}>
                {o.organisation_name}
              </Box>
            ))}
      ...
      <DialogContent sx={{ px: "24px", pt: "20px", pb: 0 }}>
        {successMsg && (
          <Alert severity="success" sx={{ mb: "14px", fontSize: 12, borderRadius: "8px" }}>{successMsg}</Alert>
        )}

        <Typography fontSize={11} fontWeight={700} color="#9CA3AF" letterSpacing="0.08em"
          mb="10px" sx={{ textTransform: "uppercase" }}>
          Select Organisation
        </Typography>
        ...
        <Box sx={{ border: `1px solid ${C.border}`, borderRadius: "10px", maxHeight: 280, overflowY: "auto" }}>
          {orgsLoading ? (
            <Box display="flex" justifyContent="center" py={4}><CircularProgress size={20} sx={{ color: C.accent }} /></Box>
          ) : filteredOrgs.length === 0 ? (
            <Box py={4} textAlign="center"><Typography fontSize={13} color={C.textSecondary}>No organisations found.</Typography></Box>
          ) : (
            filteredOrgs.map((org, idx) => {
              ...
              return (
                <ListItemButton key={id}
                  ...
                  <Box sx={{ flex: 1 }}>
                    <Typography fontSize={13} fontWeight={isSelected ? 600 : 400} color={C.textPrimary}>{name}</Typography>
                    <Typography fontSize={11} color="#9CA3AF">{id?.slice(0, 8)}…</Typography>
                  </Box>
                  {isAssigned && (
                    <Box sx={{ fontSize: 10, fontWeight: 600, px: "6px", py: "2px", borderRadius: "6px", backgroundColor: C.greenSoft ?? "#E7F8EE", color: "#0F6E56" }}>
                      Assigned
                    </Box>
                  )}
                </ListItemButton>
      ...
      <DialogActions sx={{ px: "24px", py: "20px", gap: 1.5 }}>
        <Button fullWidth variant="outlined" onClick={handleClose}
          sx={{ textTransform: "none", fontWeight: 600, fontSize: 13, borderColor: C.border, color: C.textSecondary, borderRadius: "10px", "&:hover": { borderColor: "#D1D5DB", backgroundColor: "#F9FAFB" } }}>
          Cancel
        </Button>
        <Button fullWidth variant="contained" onClick={handleAssign} disabled={!selectedOrg || assigning}
          sx={{ textTransform: "none", fontWeight: 600, fontSize: 13, backgroundColor: C.accent, borderRadius: "10px", boxShadow: "none", "&:hover": { backgroundColor: "#E54E10", boxShadow: "none" }, "&:disabled": { backgroundColor: "#F3F4F6", color: "#9CA3AF" } }}>
          {assigning
