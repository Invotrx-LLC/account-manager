...
const T = {
  labelFontWeight:600,
  labelSize: 14,   // section/field labels
  valueSize: 13,   // field values
};
    ...
    >
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          {/* Accordion title = label size */}
          <Typography fontSize={T.labelSize} fontWeight={700} color={C.textPrimary}>{title}</Typography>
        </Box>
        {open
      ...
      <Collapse in={open}>
        <Box sx={{ px: "20px", pb: "20px", borderTop: `1px solid ${C.border}`, pt: "16px" }}>
          {children}
        </Box>
      </Collapse>
    </Box>
    ...
    <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <Box sx={{ display: "flex", gap: "16px", mt: "4px" }}>
        {[
          ...
          <Box key={label} sx={{ display: "flex", alignItems: "center", gap: "6px" }}>
            <Box sx={{ width: 10, height: 10, borderRadius: "50%", backgroundColor: dot }} />
            <Typography fontSize={T.valueSize} color={C.textSecondary} fontWeight={600}>{label}</Typography>
          </Box>
        ))}
      </Box>
    </Box>
  ...
  return (
    <Box>
      ...
      {summary && (
        <Box sx={{ mb: "16px" }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: "8px", mb: "8px" }}>
            ...
            <Typography fontSize={T.labelSize} fontWeight={700} color={C.textPrimary}>Skills Summary</Typography>
          </Box>
          {/* Summary text = value */}
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {Object.entries(skillNotes).map(([key, note]) => (
            <Box key={key} sx={{ display: "flex", gap: "6px" }}>
              {/* key = label */}
              <Typography color={C.textPrimary} sx={{ minWidth: "fit-content", fontWeight:T.labelFontWeight,fontSize:T.labelSize}}>
                {key} :
              </Typography>
              {/* note = value */}
              <Typography sx={{fontSize:T.valueSize}} lineHeight={1.65}>{note}</Typography>
            </Box>
          ))}
        </Box>
      )}
      ...
      {skillInfo.skillintel_score != null && (
        <Box sx={{ mt: "16px", display: "inline-flex", alignItems: "center", gap: "8px", backgroundColor: C.accentSoft, border: "1px solid #FFCFB3", borderRadius: "10px", px: "14px", py: "8px" }}>
      ...
      >
        <DialogTitle sx={{ px: "20px", py: "14px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <Box sx={{ width: 32, height: 32, borderRadius: "8px", backgroundColor: C.accentSoft, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <TrendingUpIcon sx={{ fontSize: 16, color: C.accent }} />
            </Box>
            <Box>
              {/* Dialog title = label */}
              <Typography fontSize={T.labelSize} fontWeight={700} color={C.textPrimary}>All Skill Scores</Typography>
              {/* Sub-label = value */}
          ...
          <IconButton size="small" onClick={() => setSkillScoresOpen(false)} sx={{ color: "#9CA3AF", "&:hover": { backgroundColor: "#F3F4F6" } }}>
            <CloseIcon fontSize="small" />
          </IconButton>
        </DialogTitle>

        <Box sx={{ px: "20px", py: "10px", borderBottom: `1px solid ${C.border}`, backgroundColor: "#FAFAFA", display: "flex", gap: "16px", flexWrap: "wrap" }}>
          {[
        ...
        {/* Column headers = label size */}
        <Box sx={{ display: "grid", gridTemplateColumns: "1fr 90px 90px 64px", px: "20px", py: "8px", backgroundColor: "#F9FAFB", borderBottom: `1px solid ${C.border}` }}>
          {["Skill", "Candidate", "Desired", "Match"].map((h) => (
            <Typography key={h} fontSize={T.labelSize} fontWeight={700} color="#9CA3AF" sx={{ textTransform: "uppercase", letterSpacing: "0.06em" }}>{h}</Typography>
          ))}
        </Box>
        ...
        <DialogContent sx={{ p: 0, maxHeight: 420, overflowY: "auto" }}>
          {scoreIntel.map((s, i) => {
            ...
            return (
              <Box key={s.skill} sx={{ display: "grid", gridTemplateColumns: "1fr 90px 90px 64px", px: "20px", py: "11px", alignItems: "center", borderBottom: isLast ? "none" : `1px solid #F3F4F6`, backgroundColor: i % 2 === 0 ? "#fff" : "#FAFAFA" }}>
                {/* Skill name = label */}
                <Typography fontSize={T.labelSize} fontWeight={500} color={C.textPrimary} sx={{ pr: "8px", lineHeight: 1.3 }}>{s.skill}</Typography>
                <Box sx={{ display: "flex", alignItems: "center", gap: "6px", pr: "8px" }}>
                  ...
                  {/* Score value = value size */}
                  <Typography fontSize={T.valueSize} fontWeight={700} color={barColor} sx={{ minWidth: 24, textAlign: "right" }}>{s.candidate}</Typography>
                </Box>
                <Box sx={{ display: "flex", alignItems: "center", gap: "6px", pr: "8px" }}>
                  <Box sx={{ flex: 1, height: 5, backgroundColor: "#F3F4F6", borderRadius: 3 }}>
                    <Box sx={{ height: "100%", width: `${Math.min(s.desired, 100)}%`, backgroundColor: "#22C55E", borderRadius: 3 }} />
                  </Box>
                  <Typography fontSize={T.valueSize} fontWeight={700} color="#22C55E" sx={{ minWidth: 24, textAlign: "right" }}>{s.desired}</Typography>
                </Box>
                <Box sx={{ display: "inline-flex", alignItems: "center", justifyContent: "center", px: "8px", py: "2px", borderRadius: "20px", backgroundColor: `${barColor}18`, border: `1px solid ${barColor}40` }}>
                  <Typography fontSize={T.valueSize} fontWeight={700} color={barColor}>{pct}%</Typography>
                </Box>
              </Box>
            );
          })}
        ...
        <Box sx={{ px: "20px", py: "12px", borderTop: `1px solid ${C.border}`, backgroundColor: "#FAFAFA", display: "flex", gap: "20px" }}>
          {[
...
function SkillsSection({ candidate }) {
  ...
  const skills = keySkills ? keySkills.split(",").map((s) => s.trim()).filter(Boolean) : [];
        ...
        {skills.map((skill) => {
          ...
          return (
            <Chip key={skill} label={skill} size="small"
              sx={{ fontSize:10,  height: 26, backgroundColor: style.bg, color: style.color, border: `1px solid ${style.border}`, borderRadius: "6px", "& .MuiChip-label": { px: "10px" } }}
            />
          );
        })}
      </Box>
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      {employment.map((emp, i) => (
        <Box key={emp.id ?? i} sx={{ p: "16px", border: `1px solid ${C.border}`, borderRadius: "10px", backgroundColor: emp.is_current ? "#FFFBF8" : "#fff", borderLeft: emp.is_current ? `3px solid ${C.accent}` : `1px solid ${C.border}` }}>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: "6px" }}>
            <Box>
              {/* Job title = label */}
              <Typography sx={{fontSize:T.labelSize}} color={C.textPrimary}>{emp.job_title}</Typography>
              {/* Company + location = value */}
              <Typography sx={{fontSize:T.valueSize}} mt="2px">
                {emp.company_name}{emp.location ? ` · ${emp.location}, ${emp.country}` : ""}
              </Typography>
            </Box>
            {emp.is_current && (
              <Chip label="Current" size="small" sx={{ fontSize: 10, fontWeight: 700, height: 20, backgroundColor: C.accentSoft, color: C.accent, "& .MuiChip-label": { px: "8px" } }} />
            )}
          </Box>
          {/* Dates = value */}
          <Box sx={{ display: "flex", gap: "8px", mb: "10px", flexWrap: "wrap" }}>
            ...
            {emp.duration && <Typography sx={{ fontSize: T.valueSize, color: "#9CA3AF" }}>· {emp.duration}</Typography>}
            {(emp.total_exp_years || emp.total_exp_months) && (
              <Typography sx={{ fontSize: T.valueSize, color: "#9CA3AF" }}>· {emp.total_exp_years}y {emp.total_exp_months}m</Typography>
            )}
          </Box>
          {emp.skills_used?.length > 0 && (
            <Box sx={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
              {emp.skills_used.slice(0, 12).map((sk) => (
                <Chip key={sk} label={sk} size="small" sx={{ fontSize: 10, height: 22, backgroundColor: "#F3F4F6", color: "#374151", borderRadius: "5px", "& .MuiChip-label": { px: "8px" } }} />
              ))}
...
function EducationSection({ candidate }) {
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: "12px" }}>
      {education.map((edu, i) => (
        <Box key={edu.id ?? i} sx={{ display: "flex", gap: "14px", p: "14px", border: `1px solid ${C.border}`, borderRadius: "10px", backgroundColor: "#fff" }}>
          <Box sx={{ minWidth: 52, height: 52, borderRadius: "10px", backgroundColor: C.tealSoft, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            {/* Year badge = label (prominent) */}
            <Typography sx={{fontSize:T.labelSize}} color={C.teal}>{edu.end_year ?? "—"}</Typography>
          </Box>
          <Box>
            {/* Course = label */}
            <Typography sx={{fontSize:T.labelSize, fontWeight: 700, color: C.textPrimary}}>{edu.course}</Typography>
            {/* University = value */}
            <Typography sx={{ fontSize: T.valueSize, color: C.textSecondary, mt: "2px" }}>{edu.university}</Typography>
            {edu.specialization && (
              <Typography sx={{ fontSize: T.valueSize, color: "#9CA3AF", mt: "2px" }}>{edu.specialization}</Typography>
            )}
          </Box>
        </Box>
      ))}
  ...
  return (
    <Grid container spacing={4}>
      ...
      <Grid size={{ xs: 12, md: 6 }}>
        ...
        <Typography fontSize={T.labelSize} fontWeight={700} color="#9CA3AF" letterSpacing="0.08em" mb="20px"
              ...
              <Box key={stage.key} sx={{ display: "flex", gap: "16px" }}>
                ...
                <Box sx={{ pb: isLast ? 0 : "12px", width: "100%", pt: "4px" }}>
                  ...
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    <Typography sx={{ fontSize: T.labelSize }} color={C.textSecondary}>IST :</Typography>
      ...
      <Grid size={{ xs: 12, md: 6 }}>
        ...
        <Box sx={{ border: `1px solid ${C.border}`, borderRadius: "12px", overflow: "hidden", backgroundColor: "#fff" }}>
          {[
            ...
            <Box key={label} sx={{ display: "flex", justifyContent: "space-between", px: "16px", py: "12px", borderBottom: idx < arr.length - 1 ? "1px solid #F3F4F6" : "none" }}>
              {/* Field label = label size */}
              <Typography fontSize={T.labelSize} color={C.textSecondary}>{label}</Typography>
              {/* Field value = value size */}
              <Typography fontSize={T.valueSize} fontWeight={500} color={C.textPrimary} sx={{ textTransform: "capitalize" }}>
                {value ?? "—"}
              </Typography>
            </Box>
          ))}
        ...
        {candidate.upcoming_interview && (
          <Box sx={{ mt: "16px", p: "14px", border: `1px solid ${C.border}`, borderRadius: "10px", backgroundColor: "#fff" }}>
            ...
            <Box onClick={onInterviewClick} sx={{ display: "flex", alignItems: "center", gap: 0.5, cursor: "pointer" }}>
              <Typography fontSize={T.valueSize} fontWeight={700} color={C.accent} sx={{ textTransform: "uppercase", letterSpacing: "0.05em" }}>
                Open interview
              </Typography>
      ...
      {interviews.map((iv, i) => (
        <Box key={i} sx={{ p: "16px", border: `1px solid ${C.border}`, borderRadius: "10px", backgroundColor: "#fff" }}>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: "8px" }}>
            {/* Round name = label */}
            <Typography fontSize={T.labelSize} fontWeight={700} color={C.textPrimary}>{iv.round_name}</Typography>
            <Chip label={iv.status} size="small"
              sx={{ fontSize: 10, height: 20, textTransform: "capitalize", fontWeight: 600, backgroundColor: iv.status === "scheduled" ? "#E7F8EE" : "#F3F4F6", color: iv.status === "scheduled" ? "#0F6E56" : "#6B7280", "& .MuiChip-label": { px: "8px" } }}
            />
          </Box>
          {/* Date/time = value */}
          <Typography fontSize={T.valueSize} color={C.textSecondary}>
          ...
          {iv.interviewer && (
            /* Interviewer label + value inline */
            <Typography fontSize={T.valueSize} color={C.textSecondary} mt="2px">
              <Box component="span" sx={{ fontSize: T.labelSize, fontWeight: 600, color: C.textPrimary }}>Interviewer: </Box>
              {iv.interviewer}
            </Typography>
          )}
        </Box>
  ...
  return (
    <Box sx={{ p: "16px", border: `1px solid ${C.border}`, borderRadius: "10px", backgroundColor: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center", maxWidth: 500 }}>
      ...
      <Button size="small" variant="outlined" href={candidate.resume_url} target="_blank"
        sx={{ borderColor: C.accent, color: C.accent, textTransform: "none", fontWeight: 600, borderRadius: "8px", fontSize: T.valueSize, flexShrink: 0, ml: 2, "&:hover": { backgroundColor: C.accentSoft, borderColor: C.accent } }}>
        View
      </Button>
    </Box>
  );
  ...
  return (
    <Box sx={{ p: 1, backgroundColor: "#fff" }}>
      <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: "4px" }}>
        <Box onClick={() => navigate(-1)} sx={{ cursor: "pointer", display: "flex", alignItems: "center", color: "#6B7280", "&:hover": { color: "#111827" } }}>
          <ArrowBackIcon sx={{ fontSize: 18 }} />
        </Box>
        {/* Candidate name = prominent heading, keep 20px */}
